...
struct AtlasLevelNode: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel(unlocked ? Copy.format("atlas.node.a11y", level.number, level.title, progress.stars) : Copy.format("atlas.node.locked", level.number))
        ...
        VStack(spacing: 13) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    HStack(spacing: 5) {
                        ...
                        if !level.movers.isEmpty {
                            ...
                            AtlasMapStat(icon: "circle.hexagongrid.fill", value: hasFixedCore ? Copy.text("atlas.stat.core") : "\(level.movers.count)", tint: .orange)
                        }
            ...
            HStack(spacing: 8) {
                HStack(spacing: 5) {
                    ...
                    Text(progress.stars == 3 ? Copy.text("atlas.seals.all") : Copy.format("result.seals", progress.stars))
                        .font(.system(size: 8, weight: .black, design: .rounded))
                        .tracking(0.7)
                        .foregroundStyle(EchoTheme.muted)
                }
                ...
                if let best = progress.bestTime {
                    Label(Copy.format("unit.seconds", Copy.seconds(best)), systemImage: "timer")
            ...
            Button(action: onPlay) {
                HStack {
                    ...
                    Text(Copy.text(unlocked ? "atlas.enter" : "atlas.locked"))
                        .font(.system(size: 11, weight: .black, design: .rounded))
                        .tracking(1)
                    Spacer()
                    Text(unlocked ? level.title.uppercased() : Copy.format("atlas.required", max(1, level.number - 1)))
