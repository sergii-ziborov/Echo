...
final class GameSession {
    ...
    var replayClock: TimeInterval = 0
    var balletFrames: [RenderFrame] = []
    ...
    func startBallet(_ result: SessionResult) {
        ...
        balletFrames = sim.reviewFrames
        phase = .ballet(result)
    }

    func currentBalletFrame() -> RenderFrame? {
        RenderFrame.nearest(in: balletFrames, time: balletPlaybackTime)
    }
    ...
    func advanceReplay(dt: TimeInterval) -> Bool {
        guard phase == .replaying, !replaySnapshots.isEmpty else { return false }
        replayClock += dt * ReplayTiming.rate
        let times = replaySnapshots.map(\.time)
        replayIndex = ReplayTiming.index(times: times, elapsed: replayClock)
        let start = times[0]
        let last = times[times.count - 1]
        if start + replayClock >= last - 0.000_001 {
            replayIndex = times.count - 1
